...
void cSprite::Render(FastLED_NeoMatrix *Matrix)
{
  ...
  for (int16_t i=m_Height; i>0; --i,--y)
  {
    ...
    for (j=0; j<m_Width; ++j,++x,sSh-=m_BitsPixel)
    {
      ...
      if (col > 0)
        (*Matrix).drawPixel(x, y,m_ColTable[col - 1]);
    }
    if (m_BitsPixel < 8)
    {
      while ((j % 8) != 0)
...
void cSprite::EdgeDetect(FastLED_NeoMatrix *Matrix)
{
...
{
  m_Frame = max(m_NumFrames - 1, (int)Frame);
...
cLEDSprites::cLEDSprites(FastLED_NeoMatrix *Matrix)
{
  m_Matrix = Matrix;
  m_SpriteHead = NULL;
}
        ...
        if ((colSpr[0]->m_Options & SPRITE_DETECT_COLLISION) || (colSpr[1]->m_Options & SPRITE_DETECT_COLLISION))
        {
          if ( (colSpr[0]->m_Mask != NULL) && (colSpr[1]->m_Mask != NULL) )
          {
              ...
              && (colSpr[0]->m_Y < (colSpr[1]->m_Y + colSpr[1]->m_Height)) && (colSpr[1]->m_Y < (colSpr[0]->m_Y + colSpr[0]->m_Height)) )
            {
              ...
              colXBit = xoff % 8;
              colXCnt = min((u_int16_t)(colSpr[partX]->m_Width - xoff), colSpr[1 - partX]->m_Width);
              ...
              colYCnt = min((u_int16_t)(colSpr[partY]->m_Height - yoff), colSpr[1 - partY]->m_Height);
